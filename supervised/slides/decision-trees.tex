...
\begin{document}
	...
	% Table of Contents
	\begin{frame}\textbf{Answer: D) All of the above} - The search space is exponentially large, making brute force optimization computationally intractable.

\end{popquizbox}
\end{frame}

\begin{frame}\item For examples, we have 9 Yes, 5 No
...
%\begin{frame}$H(X) = -\sum_{i=1}^k p(x_i) \log_2 p(x_i)$
...
\begin{frame}\item Can we use Outlook as the root node?
...
\begin{frame}\textbf{Answer: B) The impurity or ``disagreement'' in a set of examples} - Higher entropy means more mixed classes, lower entropy means more pure subsets.

\end{popquizbox}
\end{frame}

\begin{frame}\begin{column}{.32\textwidth}
	\begin{table}
...
\begin{frame}\item Gain($S_{\text{Outlook=Sunny}}$, Temp) = Entropy(2 Yes, 3 No) - (2/5)*Entropy(0 Yes, 2 No) -(2/5)*Entropy(1 Yes, 1 No) - (1/5)*Entropy(1 Yes, 0 No) 
...
\begin{frame}Prediction for $<$High Humidity, Strong Wind, Sunny Outlook, Hot Temp$>$ is ? \\
\pause  No
\end{frame}

\begin{frame}Apply the same rules, except when depth limit is reached, the leaf node is assigned the most common occurring value in that path.
...
\begin{frame}\textbf{Answer: B) When Outlook=Overcast, all examples have Play=Yes} - This creates a pure subset with entropy=0, maximizing information gain.
...
\begin{frame}\textbf{Answer: C) Mean Squared Error (MSE) Reduction} - For regression, we minimize MSE instead of maximizing information gain.

\end{popquizbox}
\end{frame}

\begin{frame}\textbf{Answer: B) Use midpoints between consecutive sorted feature values} - This ensures we test all meaningful boundaries between different class regions.
...
\begin{frame}\textbf{Answer: C) The mean of target values in that region} - Each leaf predicts the average target value of training samples that reach that leaf.
