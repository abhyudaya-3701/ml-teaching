...
\begin{align*}
s^* &= \operatorname*{\arg\min}_{s}
\left[
  \sum_{x_i \in R_1(s)} \bigl(y_i - c_1(s)\bigr)^2
  + \sum_{x_i \in R_2(s)} \bigl(y_i - c_2(s)\bigr)^2
\right],
\end{align*}
}
\end{frame}
...
\end{enumerate}}{
{\color{magenta}\textbf{C) The mean of target values in that region}} - Each leaf predicts the average target value of training samples that reach that leaf.
}
