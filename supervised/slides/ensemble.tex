...
\begin{keypointsbox}{Core Intuition}
\begin{itemize}
...
\begin{keypointsbox}{Tree 1 Feature Usage}
Tree 1 uses: $X_1$, $X_2$, and $X_3$ (but NOT $X_4$)
\end{keypointsbox}
\end{frame}
...
\begin{keypointsbox}{Root Node Property}
Root node always has $p(\text{root}) = 1.0$ (all samples start here)
\end{keypointsbox}
\end{frame}
...
\begin{keypointsbox}{Remaining Steps}
\begin{itemize}
...
$$\text{Importance}(X_1) = \frac{1}{M} \sum_{m=1}^M \sum_{t \in \text{Tree}_m} \indicator[j_t = 1] \cdot p(t) \cdot \Delta i(t)$$
\end{definitionbox}
\end{frame}
