...
\begin{frame}{Intuition}
  ...
  \pause 2) Computational: Even if data is sufficient, some \textbf{classifiers/regressors can get stuck in local optima or apply greedy strategies}. Computationally learning the ``best'' hypothesis can be non-trivial.
...
\begin{frame}{Intuition}
  ...
  \pause 3) Representational: Some \textbf{classifiers/regressors cannot learn the true form or representation.}
...
\begin{frame}{Some calculations}
  % Table for Error Probability = 0.3
  \begin{center}
  \begin{tabular}{c|c|c}
...
\begin{frame}{Some calculations}
  % Table for Error Probability = 0.6
  \begin{center}
  \begin{tabular}{c|c|c}
  Number of Models & Ensemble Error & Individual Error \\
  \hline
...
\begin{frame}{Bagging}
  \only<1->{
    Consider our dataset has $n$ samples: $D_1, D_2, D_3, \ldots, D_n$.\\
