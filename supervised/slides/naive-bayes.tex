...
\begin{frame}{Bayesian Networks}
    % $$
    %     \Prob(Grass = Wet, Rain  = True, Sprinkler = True) =
    %     \\\Prob(G = W| S = T,R = T)\Prob(S = T | R = T)\Prob(R = T)
    % $$
\end{frame}
...
\begin{frame}{Solution}
    \[
        \begin{split}
            \Prob(A|T) &= \frac{\Prob(A,T)}{\Prob(A)}\\
            &=\frac{\Prob(A,T,E)+\Prob(A,T,\bar{E})}{\Prob(A,T,E)+\Prob(A,T,\bar{E}) + \Prob(A,\bar{T},E)+\Prob(A,\bar{T},\bar{E})}
        \end{split}
    \]
\end{frame}
...
\begin{frame}{Medical Diagnosis}
    ...
    \begin{itemize}
        \item $\Prob(Test = +ve | Disease = True) = 0.99$
        \item $\Prob(Test = -ve | Disease = False) = 0.99$
    \end{itemize}
...
\begin{frame}{Problem}
    \begin{itemize}
        \item $\Prob(T|D)$ = 0.99
        \item $\Prob(\bar{T}|\bar{D})$ = 0.99
        \item $\Prob(T|\bar{D})$ = 0.01
        \item $\Prob(\bar{T}|D)$ = 0.01
        \item $\Prob(D)$ =$ 10^{-4}$
        \item $\Prob(\bar{D})$ =$ 1 - 10^{-4}$
    \end{itemize}
    
    Given the above, calculate $\Prob(D|T)$. Given the result is positive, what is the probability that someone has the disease
\end{frame}
...
\begin{frame}{Problem}
    ...
    \label{eqn*:eqlabel}
        \begin{split}
            \Prob(D|T) &= \frac{\Prob(T|D)\Prob(D)}{\Prob(T)}\\
            &=\frac{\Prob(T|D)\Prob(D)}{\Prob(T|D)\Prob(D) + \Prob(T|\bar{D})\Prob(\bar{D})}
        \end{split}
    \end{align}
...
%\begin{frame}{Spam Classification}
%    
%    Calculate, $\Prob(y=1 \vert w_{1}=0, w_{2}=0, w_{3}=1)$ and $\Prob(y=1 \vert w_{1}=0, w_{2}=0, w_{3}=1)$
%    
%    if $\Prob(y=1 \vert w_{1}=0, w_{2}=0, w_{3}=1)$ $\gt$ $\Prob(y=0 \vert w_{1}=0, w_{2}=0, w_{3}=1)$, then it is a spam mail
%\end{frame}
...
% %     \item $\Prob(A|B)\Prob(B) = \Prob(B|A)\Prob(A)$
...
% % \begin{itemize}
% % 	\item $\Prob(\theta|\mathcal{D}) = \frac{\Prob(\mathcal{D}|\theta)\Prob(\theta)}{\Prob(\mathcal{D})}$
% % 	\item Posterior: 
% % 	\item Prior:
% % 	\item Likelihood
% % 	\item 
...
% % 	\item Given a coin flip and 5 H and 1 T, what is more likely: \Prob(H) = 0.5 or \Prob(H) = 1
% % \end{itemize}
% % \end{frame}
...
% % 	\item Let us have $\Prob(H) = \theta, \Prob(T) = 1-\theta$
% % 	\item We have Likelihood, $L(\theta) = \Prob(\mathcal{D}|\theta) = \Prob(\data_1, \data_2, ..., \data_N|\theta)$
% % 	\item Since observations are i.i.d., $L(\theta) = \Prob(\data_1|\theta).\Prob(\data_2|\theta) ... \Prob(\data_N|\theta)$
% % \end{itemize}
...
% % \begin{tcolorbox}
% % 	\begin{align}
% % \hat{\theta}_{MAP} = \argmin_\theta \Prob(\theta|\data)\\
% % \hat{\theta}_{MAP}= \argmin_\theta \Prob(\data|\theta)\Prob(\theta)
% % \end{align}
% % \end{tcolorbox}
