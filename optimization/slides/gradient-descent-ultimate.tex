...
\begin{document}
  ...
  \begin{frame}{Visual Intuition 1: Overall Loss Surface}
    ...
    \begin{center}
    \begin{tikzpicture}[scale=1.2]
      % Overall loss contours (elliptical)
      \draw[blue, very thick] (0,0) ellipse (3 and 1.8);
      \draw[blue, very thick] (0,0) ellipse (2.2 and 1.3);
      \draw[blue, very thick] (0,0) ellipse (1.5 and 0.9);
      \draw[blue, very thick] (0,0) ellipse (0.8 and 0.5);

      % Current point theta
      \coordinate (theta) at (2,1);
      \fill[red] (theta) circle (0.08);
      \node[red] at (2.3,1.2) {$\vtheta$};

      % True gradient vector (long, pointing toward center)
      \draw[-{Latex[scale=2]}, red, line width=3pt] (theta) -- +(-1.5,-0.75);
      \node[red] at (0.8,1.8) {\textbf{True gradient}};
      \node[red] at (0.8,1.5) {$\nabla L(\vtheta)$};
      \node[red] at (0.8,1.2) {\small (all data)};

      % Global minimum
      \fill[green] (0,0) circle (0.06);
      \node[green] at (0,-0.4) {\small minimum};

      \node[blue] at (-2.5,1.5) {\small Loss $L(\vtheta)$};
    \end{tikzpicture}
    \end{center}
    
    \begin{keypointsbox}{}
    {\small This is what we want: gradient computed using ALL data points}
    \end{keypointsbox}
  \end{frame}
