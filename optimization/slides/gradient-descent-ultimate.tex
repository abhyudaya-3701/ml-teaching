...
\begin{document}
  ...
  \begin{frame}{Step 2a: Setting up the Gradient Computation}
    \textbf{Starting with our total loss function:}
    
    \begin{center}
    \begin{tikzpicture}[
        node distance=1.5cm,
        loss/.style={rectangle, draw=orange, fill=orange!10, thick, minimum width=2cm, minimum height=1cm},
        arrow/.style={-{Latex[scale=1.5]}, thick},
        scale=1.2
    ]

    % Total loss - larger and more prominent
    \node[loss] (total) at (0,1) {\large $L(\vtheta) = \frac{1}{n}\sum_{i=1}^n \ell(f(\vx_i;\vtheta),y_i)$};
    
    % Gradient operation
    \node[rectangle, draw=blue, fill=blue!10, thick, minimum width=3cm, minimum height=1cm] (grad) at (0,-1.5) {\large $\nabla L(\vtheta) = \nabla \left(\frac{1}{n}\sum_{i=1}^n \ell(f(\vx_i;\vtheta),y_i)\right)$};

    % Arrow showing the gradient operation
    \draw[arrow, red, very thick] (total) -- (grad);
    \node[red] at (1.5,0) {\textbf{Take gradient}};
    \end{tikzpicture}
    \end{center}
    
    \pause
    \begin{alertbox}{The Question}
    Can we compute this gradient more efficiently than evaluating all $n$ terms?
    \end{alertbox}
  \end{frame}

  \begin{frame}{Step 2b: Gradient Linearity - The Key Mathematical Insight}
    \textbf{The magic of linearity:}
    
    \begin{center}
    \begin{tikzpicture}[
        node distance=1.5cm,
        sample/.style={rectangle, draw=blue, fill=blue!10, thick, minimum width=1.4cm, minimum height=0.8cm},
        loss/.style={rectangle, draw=orange, fill=orange!10, thick, minimum width=1.4cm, minimum height=0.8cm},
        grad/.style={rectangle, draw=red, fill=red!10, thick, minimum width=1.4cm, minimum height=0.8cm},
        sum/.style={circle, draw=green, fill=green!10, thick, minimum width=1.2cm, minimum height=1.2cm},
        arrow/.style={-{Latex[scale=1.2]}, thick},
        scale=0.8
    ]

    % Individual losses  
    \node[loss] (l1) at (-2,1) {\scriptsize $\ell_1(\vtheta)$};
    \node[loss] (l2) at (0,1) {\scriptsize $\ell_2(\vtheta)$};
    \node[loss] (l3) at (2,1) {\scriptsize $\ell_n(\vtheta)$};

    % Individual gradients
    \node[grad] (g1) at (-2,-1) {\scriptsize $\nabla\ell_1$};
    \node[grad] (g2) at (0,-1) {\scriptsize $\nabla\ell_2$};  
    \node[grad] (g3) at (2,-1) {\scriptsize $\nabla\ell_n$};

    % Sum operation
    \node[sum] (gradsum) at (4,0) {\scriptsize $\frac{1}{n}\sum$};
    
    % Final gradient
    \node[grad, fill=green!20] (final) at (6.5,0) {\scriptsize $\nabla L$};

    % Arrows
    \draw[arrow] (l1) -- (g1);
    \draw[arrow] (l2) -- (g2);
    \draw[arrow] (l3) -- (g3);
    \draw[arrow] (g1) -- (gradsum);
    \draw[arrow] (g2) -- (gradsum);
    \draw[arrow] (g3) -- (gradsum);
    \draw[arrow] (gradsum) -- (final);

    % Linearity annotation
    \node at (1,2.2) {\textbf{Linearity of $\nabla$}};
    \draw[<->, thick, blue] (-2,0.5) -- (2,0.5);
    \end{tikzpicture}
    \end{center}
    
    \begin{theorembox}{Linearity of Gradient Operator}
    $\nabla L(\vtheta) = \frac{1}{n}\sum_{i=1}^n \nabla \ell(f(\vx_i;\vtheta),y_i)$
    \end{theorembox}
  \end{frame}

  \begin{frame}{Step 3: SGD as Unbiased Estimator Implementation}
    \textbf{SGD solution:} Instead of computing all $n$ gradients, sample one!
    ...
    \node[sample, fill=yellow!30] (sgd) at (0,1) {\scriptsize Random pick};
    \node[grad, fill=yellow!30] (gsgd) at (3.5,1) {\scriptsize $\nabla\ell_i$};
    
    % True gradient reference
    \node[avg] (gtrue) at (7,1) {\scriptsize $\nabla L$ (true)};
    ...
    % Show expectation
    \node[yellow, font=\small] at (3.5,0.2) {SGD estimate};
    \node[green, font=\small] at (7,0.3) {Target};
    \node at (5.2,1.4) {\scriptsize $\E[\cdot] =$};
    \end{tikzpicture}
    \end{center}

    \begin{alertbox}{Unbiased Property: The Foundation}
    $\E[\nabla \tilde{L}(\vtheta)] = \nabla L(\vtheta)$ - SGD points in the right direction on average!
    \end{alertbox}
  \end{frame}
