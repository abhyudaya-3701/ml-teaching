...
\begin{document}
  ...
  \begin{frame}{Visual Intuition 3: Gradients from Same Starting Point}
    \textbf{What happens when we evaluate gradients from the same point $\vtheta_0$?}
    ...
    \begin{tikzpicture}[scale=0.7]
      ...
      \draw[blue, thick] (0,0) ellipse (0.5 and 0.35);
      \fill[blue] (0,0) circle (0.04);
      \node[blue, below] at (0,-1.0) {\tiny $\ell_1(\vtheta)$};
      
      % Starting point on this surface
      \fill[black] (start) circle (0.05);
      % Gradient vector from starting point for surface 1
      \draw[-{Latex[scale=1.5]}, blue, very thick] (start) -- +(-0.8, -0.3);
      \node[blue, above] at (0.4, 1.0) {\tiny $\nabla\ell_1$};
      \end{scope}
      ...
      \fill[red] (0,0) circle (0.04);
      \end{scope}
      \node[red, below] at (-1,-1.0) {\tiny $\ell_2(\vtheta)$};
      
      % Starting point and gradient for surface 2
      \fill[black] (start) circle (0.05);
      \draw[-{Latex[scale=1.5]}, red, very thick] (start) -- +(-1.2, -0.8);
      \node[red, above] at (0.8, 0.5) {\tiny $\nabla\ell_2$};
      ...
      \fill[green] (0,0) circle (0.04);
      \end{scope}
      \node[green, below] at (1,-0.9) {\tiny $\ell_3(\vtheta)$};
      
      % Starting point and gradient for surface 3
      \fill[black] (start) circle (0.05);
      \draw[-{Latex[scale=1.5]}, green, very thick] (start) -- +(-0.6, -1.0);
      \node[green, left] at (1.2, 0.2) {\tiny $\nabla\ell_3$};
      ...
      \fill[purple] (0,0) circle (0.04);
      \end{scope}
      \node[purple, below] at (3,-1.0) {\tiny $\ell_n(\vtheta)$};
      
      % Starting point and gradient for surface n
      \fill[black] (start) circle (0.05);
      \draw[-{Latex[scale=1.5]}, purple, very thick] (start) -- +(-0.4, -0.6);
      \node[purple, right] at (2.2, 0.7) {\tiny $\nabla\ell_n$};
      
      % Label starting point
      \node[black, font=\bfseries] at (1.5, 1.4) {$\vtheta_0$};
    \end{tikzpicture}
    \end{center}
    
    \pause
    \begin{theorembox}{Key Insight}
    {\small From the \textbf{same point}, each loss surface gives a \textbf{different gradient direction}!}
    \end{theorembox}
  \end{frame}

  \begin{frame}{Visual Intuition 4: Averaging Individual Gradients}
